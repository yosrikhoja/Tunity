...
function AnimatedProgressRing({ value, duration = 1.6 }: { value: number; duration?: number }) {
  const { ref, value: display } = useCountUp<SVGSVGElement>(value, { duration, margin: "-60px" });

  const circumference = 2 * Math.PI * 42;
  const dash = (display / 100) * circumference;

  return (
    <svg ref={ref} viewBox="0 0 100 100" className="h-12 w-12 shrink-0 sm:h-16 sm:w-16">
      ...
      <div className="container-tunity">
        <div className="grid items-center gap-14 lg:grid-cols-2 lg:gap-16">
          ...
          <motion.div
            ...
            className="relative"
          >
            <div className="relative aspect-[4/5] w-full overflow-hidden rounded-3xl border border-navy-100 shadow-card sm:aspect-[5/5.5]">
              <Image
                src="/Apoposimg.png"
            ...
            <div className="absolute inset-x-3 top-3 flex items-start gap-2.5 sm:inset-x-5 sm:top-5 sm:gap-3">
              <div className="min-w-0 flex-1 rounded-xl bg-white/95 p-3 shadow-card-hover backdrop-blur sm:p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wide text-navy-800 sm:text-xs">
                    <Boxes className="h-3 w-3 shrink-0 text-electric-600 sm:h-3.5 sm:w-3.5" />
                    <span className="truncate">{t.bimCard.label}</span>
                  </div>
                  <MoreHorizontal className="h-3 w-3 shrink-0 text-navy-300" />
                </div>
                <motion.ul
                  initial="hidden"
                  whileInView="show"
                ...
                >
                  {t.bimCard.items.map((item) => (
                    ...
                    >
                      ...
                      <CheckCircle2 className="h-2.5 w-2.5 shrink-0 text-emerald-500 sm:h-3 sm:w-3" />
                    </motion.li>
                  ))}
                </motion.ul>
              </div>
              ...
              <div className="w-[132px] shrink-0 rounded-xl bg-white/95 p-3 shadow-card-hover backdrop-blur sm:w-[190px] sm:p-4">
                ...
                <div className="mt-2 flex items-center gap-2 sm:mt-3 sm:gap-3">
                  <AnimatedProgressRing value={t.performanceCard.progressValue} />
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 border-t border-navy-100 pt-2.5 sm:mt-4 sm:pt-3">
                  <div>
                    ...
                    <AnimatedNumber
                      value={t.performanceCard.co2Value}
                      className="block text-xs font-bold text-navy-900 sm:text-sm"
                    />
                  ...
                  <div>
                    ...
                    <AnimatedNumber
                      value={t.performanceCard.budgetValue}
                      className="block text-xs font-bold text-navy-900 sm:text-sm"
                    />
                    <p className="text-[8px] text-navy-400 sm:text-[9px]">
          ...
          {t.stats.map((stat, i) => {
            ...
            return (
              <div key={stat.title} className="card-surface p-7">
                ...
                <AnimatedNumber
                  value={stat.value}
                  className="mt-4 block font-display text-3xl font-bold text-navy-900"
                />
